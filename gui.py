...
class SimulationGUI:
    def __init__(self, world, policy):
        ...
        self.canvas_sim = tk.Canvas(self.root, width=canvas_width, height=canvas_height, bg=BACKGROUND_COLOR)
        ...
        tk.Button(controls, text="⏸ Pause",        command=self.toggle_pause).pack(side=tk.LEFT, padx=5)
        tk.Button(controls, text="➕ Speed",         command=self.speed_up).pack(side=tk.LEFT, padx=5)
        tk.Button(controls, text="➖ Slow",          command=self.slow_down).pack(side=tk.LEFT, padx=5)
        tk.Button(controls, text="⏩ Fast Forward",  command=self.fast_forward).pack(side=tk.LEFT, padx=5)
        tk.Button(controls, text="💾 Sauvegarder",  command=self.on_save).pack(side=tk.LEFT, padx=5)
        tk.Button(controls, text="📂 Charger",      command=self.on_load).pack(side=tk.LEFT, padx=5)

        self.record_btn = tk.Button(controls, text="⏺ Enregistrer", command=self.on_record_toggle, bg="#550000", fg="white")
        self.record_btn.pack(side=tk.LEFT, padx=5)

        tk.Radiobutton(controls, text="Écran (vitesse réelle)", variable=self.record_mode, value="screen").pack(side=tk.LEFT)
        tk.Radiobutton(controls, text="Tick par tick",          variable=self.record_mode, value="tick").pack(side=tk.LEFT)
    ...
    def draw_agent_panel(self):
        ...
        from agent import (ACTION_UP, ACTION_DOWN, ACTION_LEFT, ACTION_RIGHT,
        ...
        if a.policy is not None:
            for name, info in REGISTRY.items():
                if isinstance(a.policy, info["class"]):
                    policy_name = name
                    break
    ...
    def fast_step(self):
        ...
        for _ in range(steps_per_frame):
            if len(self.world.agents) < 5:
                ...
                self.target_tick = None
                return True
            if self.target_tick is None or self.world.tick >= self.target_tick:
                self.fast_mode = False
                self.target_tick = None
                return True
    ...
    def draw_biomes(self):
        ...
        for y in range(WORLD_HEIGHT):
            for x in range(WORLD_WIDTH):
                ...
                x1 = x * CELL_SIZE
                y1 = y * CELL_SIZE
                self.canvas_sim.create_rectangle(x1, y1, x1 + CELL_SIZE, y1 + CELL_SIZE, fill=color, outline="")
    ...
    def draw_grid(self):
        for x in range(WORLD_WIDTH + 1):
            self.canvas_sim.create_line(x * CELL_SIZE, 0, x * CELL_SIZE, WORLD_HEIGHT * CELL_SIZE, fill=GRID_COLOR)
        for y in range(WORLD_HEIGHT + 1):
            self.canvas_sim.create_line(0, y * CELL_SIZE, WORLD_WIDTH * CELL_SIZE, y * CELL_SIZE, fill=GRID_COLOR)
    ...
    def draw_world(self):
        ...
        ticks_since_migration = self.world.tick - self.world.last_migration_tick
        if self.world.migration_count > 0 and ticks_since_migration < 60:
        ...
        # Compte les agents par policy
        from policy_registry import REGISTRY
        policy_counts = {name: 0 for name in REGISTRY}
        for agent in self.world.agents:
            if agent.policy is not None:
                for name, info in REGISTRY.items():
                    if isinstance(agent.policy, info["class"]):
                        policy_counts[name] += 1
                        break

        self.graph.update(self.world.tick, policy_counts, food_count, self.world.death_count)
    ...
    def draw_agents(self):
        ...
        for agent in self.world.agents:
            ...
            # Bordure colorée selon la policy
            border_color = "white"
            if agent.policy is not None:
                for name, info in REGISTRY.items():
                    if isinstance(agent.policy, info["class"]):
...
class PopulationGraph:
    ...
    def _draw(self):
        ...
        for _, counts, food, deaths in self.history:
            all_vals.append(sum(counts.values()))
            all_vals.append(food)
        ...
        for name, info in self.REGISTRY.items():
            color  = info["color"]
            points = [to_xy(i, row[1].get(name, 0)) for i, row in enumerate(self.history)]
            for j in range(len(points) - 1):
                c.create_line(points[j], points[j + 1], fill=color, width=2)

        # Nourriture
        points = [to_xy(i, row[2]) for i, row in enumerate(self.history)]
        for j in range(len(points) - 1):
            c.create_line(points[j], points[j + 1], fill="#90ee90", width=1)

        # Morts
        points = [to_xy(i, row[3]) for i, row in enumerate(self.history)]
        for j in range(len(points) - 1):
            c.create_line(points[j], points[j + 1], fill="#ff6666", width=1)
